...
NOMINATIM_PBF_URL=${NOMINATIM_PBF_URL:="http://download.geofabrik.de/europe/norway-latest.osm.pbf"}
...
if [ "$NOMINATIM_MODE" == "CREATE" ]; then
    ...
    cp -R $NOMINATIM_DATA_PATH/*.tgz* /mnt/azure/$NOMINATIM_DATA_LABEL
    ...
    cp -R /mnt/azure/$NOMINATIM_DATA_LABEL/*.tgz* $NOMINATIM_DATA_PATH
